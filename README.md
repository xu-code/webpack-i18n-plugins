...
2、安装完成之后，先直接启动项目，插件会将历史的index.xlsx转化为user.json文件，与此同时，旧的index.xlsx将会被删除，如有需要，请自行备份。
...
const i18nConfig = {
    ...
    translation: {
        en_US: {
            userJson: path.resolve(__dirname, './i18n/en_US/user.json'), // 对应的翻译文件
            // formatter: value => value + ' ' // 根据自身需求开启
        }
    },
...
[i18n/en/index.json](i18n/en/index.json)
...
[i18n/en/user.json](i18n/en/user.json)
...
根据上面的配置（配置了userJson，formatter），最终生成的语言包中，[开发] 会被翻译为 <u>[Develop ]</u>；[测试]会被翻译为<u>[Test ]</u>。
